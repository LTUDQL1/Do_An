...
namespace QLQuanAn
{
    public partial class Salad : Form
    {
        ...
        public Salad()
        {
            ...
            dgvSalad.SelectionChanged += new EventHandler(dgvMonMan_SelectionChanged);
        ...
        {
            string str = "Select * from MON_AN where Loai = 'Salad' and MaCN like N'%"+cbbChiNhanh.SelectedValue+"'";
            dsMM = XuLyDuLieu.docDuLieu(str);
            dgvSalad.DataSource = dsMM;
            dgvSalad.Columns[0].Visible = false;
            dgvSalad.Columns[5].Visible = false;
        ...
        {
            if (dgvSalad.SelectedRows.Count > 0)
            {
                DataRow cn = ((DataRowView)dgvSalad.SelectedRows[0].DataBoundItem).Row;
