...
namespace QLQuanAn
{
    public partial class MonMan : Form
    {
        ...
        public MonMan()
        {
            ...
            dgvMonMan.SelectionChanged += new EventHandler(dgvMonMan_SelectionChanged);
        }
        public void loadDSMM()
        {
            string str = "Select * from MON_AN where Loai = 'MonMan' and MaCN like N'%"+cbbChiNhanh.SelectedValue+"'";
            dsMM = XuLyDuLieu.docDuLieu(str);
            dgvMonMan.DataSource = dsMM;
            dgvMonMan.Columns[0].Visible = false;
            dgvMonMan.Columns[5].Visible = false;
        ...
        {
            if (dgvMonMan.SelectedRows.Count > 0)
            {
                DataRow cn = ((DataRowView)dgvMonMan.SelectedRows[0].DataBoundItem).Row;
